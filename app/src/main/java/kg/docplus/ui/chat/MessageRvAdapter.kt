...
class MessageRvAdapter(val context: Context) : RecyclerView.Adapter<MessageRvAdapter.AdvertViewHolder>() {
    ...
    inner class AdvertViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(item: Message) = with(itemView) {
            ...
            val lineStatus: RelativeLayout = itemView.findViewById(R.id.line_status)
            ...
            if (item.status.isNullOrEmpty()) {
                ...
                when {
                    item.status==ChatConstants.request_video -> itemView.status.text = "Вы отправили запрос на видеозвонок"
                    item.status==ChatConstants.video_cancel -> itemView.status.text = "Видеозвонок отменен"
                    item.status==ChatConstants.video_success -> itemView.status.text = "Видеозвонок принят"
                }
                var date = Date()
                date.time = item.time.toLong()
                itemView.time_status.text = dateToChatFormat(date)
